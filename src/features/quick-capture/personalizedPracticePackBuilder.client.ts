/**
 * Client-side copy of personalized practice pack builder (keeps offline “From your day” in sync with API logic).
 * Mirrors `backend/src/services/quick-capture/personalizedPracticePackBuilder.ts`.
 */
import {
...
} from '@/features/quick-capture/capturePackHints'
import {
  buildListeningBurstText,
  buildReadAloudPassageForWord,
  buildWordRepExercisePrompt,
  extractWordPackHints,
} from '@/features/quick-capture/wordRepPackHints'

export type DailyCaptureClusterLite = {
  id: string
  title: string
  summary: string
  scenarioTags: string[]
...
function parseEnrichedScenario(row: CaptureForPack): string {
...
function clustersTouchingCaptures(
  clusters: readonly DailyCaptureClusterLite[],
  captureIdSet: Set<string>,
): DailyCaptureClusterLite[] {
  return clusters
...
function buildRecap(params: {
  localDate: string
  orderedCaptures: CaptureForPack[]
  topClusters: DailyCaptureClusterLite[]
  mode: PracticePackMode
  estimatedMinutes: number
}): DayPracticeStep {
...
function repStepsForCapture(row: CaptureForPack, mode: PracticePackMode, maxForThis: number): DayPracticeStep[] {
...
function pickStrongestNext(ordered: CaptureForPack[]): DayPracticeStep {
...
export function buildPersonalizedDayPracticePack(params: {
  localDate: string
  captures: CaptureForPack[]
  themeClusters: readonly DailyCaptureClusterLite[]
  ...
  const orderedFromClusters: CaptureForPack[] = []
  const seen = new Set<string>()
