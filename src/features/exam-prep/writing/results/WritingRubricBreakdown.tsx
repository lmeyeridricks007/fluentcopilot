...
export function WritingRubricBreakdown({ ui, taskId }: { ui: WritingTrainingFeedbackUi; taskId: string }) {
  ...
  const onToggle = useCallback(
    (key: string, open: boolean) => {
      ...
      track(ANALYTICS_EVENTS.writing_exam_category_viewed, {
        task_id: taskId,
        category_key: key,
        exam_mode: 'training',
      })
  ...
  return (
    <div className="space-y-2">
      ...
      <p className="text-caption text-ink-tertiary leading-snug">
        Tik een onderdeel open voor uitleg bij uw score. Dit volgt het formele schrijfmodel (A2).
      </p>
              ...
              >
                ...
                <div className="px-3 pb-3 pt-0 border-t border-slate-100 space-y-2">
                  ...
                  {row.detail && row.detail !== row.learnerFeedbackNl && row.detail !== row.evidenceNl ? (
                    <div className="pt-1">
                      <p className="text-caption font-semibold text-ink-tertiary uppercase tracking-wide">
                        Rubric-toelichting
                      </p>
                      <p className="text-caption text-ink-tertiary leading-snug mt-0.5">{row.detail}</p>
                    </div>
                  ) : null}
                </div>
              </details>
