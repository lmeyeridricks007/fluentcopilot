...
import {
  STORE_SERVICE_ISSUE_DETAIL_FOCUS_LAUNCHER_OPTIONS,
  STORE_SERVICE_ISSUE_SUBTYPE_OPTIONS,
  STORE_SERVICE_ISSUE_VARIATION_OPTIONS,
  type StoreServiceIssueDetailFocusId,
  type StoreServiceIssueScenarioOverrides,
  type StoreServiceIssueScenarioSubtype,
  type StoreServiceIssueScenarioVariation,
...
export function StoreServiceIssueScenarioControls({
  smartMode,
  overrides,
  customizeOpen,
  onSmartModeChange,
  ...
  smartMode: boolean
  overrides: StoreServiceIssueScenarioOverrides
  customizeOpen: boolean
  onSmartModeChange: (next: boolean) => void
  onOverridesChange: (next: StoreServiceIssueScenarioOverrides) => void
  onCustomizeOpenChange: (next: boolean) => void
}) {
  /** Any pinned subtype, variation, or issue focus turns off smart mix; clearing all re-enables it. */
  function syncSmartModeFromOverrides(next: StoreServiceIssueScenarioOverrides) {
  ...
  const applySubtype = (id: StoreServiceIssueScenarioSubtype) => {
    const nextSub = overrides.subType === id ? undefined : id
    const next: StoreServiceIssueScenarioOverrides = {
  ...
  const applyVariation = (id: StoreServiceIssueScenarioVariation) => {
  ...
  const applyDetailFocus = (id: StoreServiceIssueDetailFocusId) => {
        ...
        <div className="flex items-start justify-between gap-3">
          <div className="min-w-0">
            <div className="flex items-center gap-2.5">
              ...
              <div>
                <p className="text-body-sm font-semibold tracking-tight text-ink-primary">Mix up the issue each time</p>
                <p className="mt-1 max-w-prose text-caption leading-relaxed text-ink-secondary">
                  We vary the situation, problem type, and response pattern so practice stays realistic.
                </p>
              </div>
            </div>
          </div>
        ...
        >
          <div>
            ...
            <p className="mt-0.5 text-caption leading-snug text-ink-secondary">
              Optional — pick a desk type, task focus, or a typical issue. Leave folded to use smart mix.
            </p>
          </div>
          <ChevronDown
            ...
            <div className="space-y-5">
              <div>
                ...
                <div className="mt-2 flex flex-wrap gap-2">
                  {STORE_SERVICE_ISSUE_SUBTYPE_OPTIONS.map((o) => (
                    <ChoicePill
                      key={o.id}
                      label={o.label}
              ...
              <div>
                ...
                <div className="mt-2 flex flex-wrap gap-2">
                  {STORE_SERVICE_ISSUE_VARIATION_OPTIONS.map((o) => (
                    <ChoicePill
                      key={o.id}
                      label={o.label}
              ...
              <div>
                <p className="text-[11px] font-semibold uppercase tracking-wide text-ink-tertiary">Optional issue</p>
                <div className="mt-2 flex flex-wrap gap-2">
                  {STORE_SERVICE_ISSUE_DETAIL_FOCUS_LAUNCHER_OPTIONS.map((o) => (
                    <ChoicePill
                      key={o.id}
                      label={o.label}
