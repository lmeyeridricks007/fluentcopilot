...
import {
  buildPartySocialRecapHookBundle,
  inferPartySocialGoalLabelsFromUserText,
  partySocialCompletionContractSatisfied,
...
export { inferPartySocialGoalLabelsFromUserText }

export function reconcilePartySocialLiveRecap(params: {
  ...
  if (normSlug(params.scenarioSlug) !== 'party_social') return params.summary
  ...
  const inferred = inferPartySocialGoalLabelsFromUserText(scenarioGoals, userMessageTexts)
  ...
  if (turnCt >= 2) {
    const flow = scenarioGoals.find((x) => x.toLowerCase().includes('beweging') || x.toLowerCase().includes('stilte'))
    if (flow) merged.add(flow)
  }
  ...
  const contractMet = partySocialCompletionContractSatisfied(runtime, goalsCompleted)
  const hooks = buildPartySocialRecapHookBundle({
    contractMet,
