...
import {
  buildWorkColleagueInteractionRecapHookBundle,
  inferWorkColleagueInteractionGoalLabelsFromUserText,
...
export { inferWorkColleagueInteractionGoalLabelsFromUserText }

export function reconcileWorkColleagueInteractionLiveRecap(params: {
  ...
  if (normSlug(params.scenarioSlug) !== 'work_colleague_interaction') return params.summary
  ...
  const inferred = inferWorkColleagueInteractionGoalLabelsFromUserText(scenarioGoals, userMessageTexts)
  ...
  const contractMet = workColleagueCompletionContractSatisfied(runtime, goalsCompleted)
  const hooks = buildWorkColleagueInteractionRecapHookBundle({
    variation,
    contractMet,
