...
import {
  buildPhoneCallRecapHookBundle,
  inferPhoneCallGoalLabelsFromUserText,
  phoneCallCompletionContractSatisfied,
...
export { inferPhoneCallGoalLabelsFromUserText }

export function reconcilePhoneCallLiveRecap(params: {
  ...
  if (normSlug(params.scenarioSlug) !== 'phone_call') return params.summary
  ...
  const inferred = inferPhoneCallGoalLabelsFromUserText(scenarioGoals, userMessageTexts)
  ...
  const contractMet = phoneCallCompletionContractSatisfied(runtime, goalsCompleted)
  const hooks = buildPhoneCallRecapHookBundle({
    contractMet,
