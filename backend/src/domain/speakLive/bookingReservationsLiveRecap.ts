...
import {
  ...
  buildBookingReservationsRecapHookBundle,
  inferBookingReservationsGoalLabelsFromUserText,
...
export { inferBookingReservationsGoalLabelsFromUserText }

export function reconcileBookingReservationsLiveRecap(params: {
  ...
  if (normSlug(params.scenarioSlug) !== 'booking_reservations') return params.summary
  ...
  const inferred = inferBookingReservationsGoalLabelsFromUserText(scenarioGoals, userMessageTexts)
  ...
  const contractMet = bookingCompletionContractSatisfied(runtime, goalsCompleted)
  const hooks = buildBookingReservationsRecapHookBundle({
    variation,
    contractMet,
