...
import {
  buildOpinionsDiscussionsRecapHookBundle,
  inferOpinionsDiscussionsGoalLabelsFromUserText,
  opinionsDiscussionsCompletionContractSatisfied,
...
export { inferOpinionsDiscussionsGoalLabelsFromUserText }

export function reconcileOpinionsDiscussionsLiveRecap(params: {
  ...
  if (normSlug(params.scenarioSlug) !== 'opinions_discussions') return params.summary
  ...
  const inferred = inferOpinionsDiscussionsGoalLabelsFromUserText(scenarioGoals, userMessageTexts)
  ...
  const contractMet = opinionsDiscussionsCompletionContractSatisfied(runtime, goalsCompleted)
  const hooks = buildOpinionsDiscussionsRecapHookBundle({
    contractMet,
  ...
  const languageNoteLines = [
    ...
    ...goalsMissed.map((g) => `discussion_point_hint:${g}`),
  ]
