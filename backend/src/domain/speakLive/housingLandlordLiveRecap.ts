...
import {
  buildHousingLandlordRecapHookBundle,
  inferHousingLandlordGoalLabelsFromUserText,
...
export { inferHousingLandlordGoalLabelsFromUserText }

export function reconcileHousingLandlordLiveRecap(params: {
  ...
  if (normSlug(params.scenarioSlug) !== 'housing_landlord') return params.summary
  ...
  const inferred = inferHousingLandlordGoalLabelsFromUserText(scenarioGoals, userMessageTexts)
  ...
  const contractMet = housingLandlordCompletionContractSatisfied(runtime, goalsCompleted)
  const hooks = buildHousingLandlordRecapHookBundle({
    variation,
    contractMet,
