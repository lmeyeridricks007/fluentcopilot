...
import {
  buildMeetingNewPeopleRecapHookBundle,
  inferMeetingNewPeopleGoalLabelsFromUserText,
  meetingNewPeopleCompletionContractSatisfied,
...
export { inferMeetingNewPeopleGoalLabelsFromUserText }

export function reconcileMeetingNewPeopleLiveRecap(params: {
  ...
  if (normSlug(params.scenarioSlug) !== 'meeting_new_people') return params.summary
  ...
  const inferred = inferMeetingNewPeopleGoalLabelsFromUserText(scenarioGoals, userMessageTexts)
  ...
  if (turnCt >= 2) {
    const intro = scenarioGoals.find((x) => x.toLowerCase().includes('beurt') || x.toLowerCase().includes('voorstel'))
    if (intro) merged.add(intro)
  }
  ...
  const contractMet = meetingNewPeopleCompletionContractSatisfied(runtime, goalsCompleted)
  const hooks = buildMeetingNewPeopleRecapHookBundle({
    contractMet,
