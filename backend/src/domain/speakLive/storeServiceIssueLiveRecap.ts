...
import {
  buildStoreServiceIssueRecapHookBundle,
  inferStoreServiceIssueGoalLabelsFromUserText,
  storeServiceCompletionContractSatisfied,
...
export { inferStoreServiceIssueGoalLabelsFromUserText }

export function reconcileStoreServiceIssueLiveRecap(params: {
  ...
  if (normSlug(params.scenarioSlug) !== 'store_service_issue') return params.summary
  ...
  const inferred = inferStoreServiceIssueGoalLabelsFromUserText(scenarioGoals, userMessageTexts)
  ...
  const contractMet = storeServiceCompletionContractSatisfied(runtime, goalsCompleted)
  const hooks = buildStoreServiceIssueRecapHookBundle({
    variation,
    contractMet,
