...
import {
  buildExplainingSomethingRecapHookBundle,
  explainingSomethingCompletionContractSatisfied,
  inferExplainingSomethingGoalLabelsFromUserText,
...
export { inferExplainingSomethingGoalLabelsFromUserText }

export function reconcileExplainingSomethingLiveRecap(params: {
  ...
  if (normSlug(params.scenarioSlug) !== 'explaining_something') return params.summary
  ...
  const inferred = inferExplainingSomethingGoalLabelsFromUserText(scenarioGoals, userMessageTexts)
  ...
  const contractMet = explainingSomethingCompletionContractSatisfied(runtime, goalsCompleted)
  const hooks = buildExplainingSomethingRecapHookBundle({
    contractMet,
  ...
  const languageNoteLines = [
    ...
    ...goalsMissed.map((g) => `step_gap_hint:${g}`),
  ]
