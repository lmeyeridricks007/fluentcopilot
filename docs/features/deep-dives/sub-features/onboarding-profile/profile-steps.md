...
Multi-step onboarding UI for language, country, family, occupation, goals; validates required fields per FD-01.
...
- Feature flags: `profile-steps` enablement, staged rollout percentage, kill switch.
...
- Analytics: `profile-steps_started`, `profile-steps_succeeded`, `profile-steps_failed` (names illustrative—align with analytics catalog).
