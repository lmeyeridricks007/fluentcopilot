# Sub-feature: Curriculum manifest
...
Manifest, units, ordered lesson IDs per locale/level; importer from CMS or JSON bundle pipeline.
...
- Feature flags: `curriculum-manifest` enablement, staged rollout percentage, kill switch.
...
- Analytics: `curriculum-manifest_started`, `curriculum-manifest_succeeded`, `curriculum-manifest_failed` (names illustrative—align with analytics catalog).
