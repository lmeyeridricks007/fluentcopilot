# Sub-feature: User path progress
...
Derive next lesson and unit completion from `lesson_progress` and manifest order.
...
- Feature flags: `user-path-progress` enablement, staged rollout percentage, kill switch.
...
- Analytics: `user-path-progress_started`, `user-path-progress_succeeded`, `user-path-progress_failed` (names illustrative—align with analytics catalog).
