...
**Domain reference**: Authentication — foundation for FD-01+  
...
Create a new account via email/password and optional OAuth; establish initial user record and session or verification flow.
...
- Feature flags: `sign-up` enablement, staged rollout percentage, kill switch.
...
- Analytics: `sign-up_started`, `sign-up_succeeded`, `sign-up_failed` (names illustrative—align with analytics catalog).
