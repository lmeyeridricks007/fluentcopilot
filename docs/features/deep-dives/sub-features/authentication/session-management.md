...
Validate, refresh, and propagate session/JWT; attach user identity to downstream requests; handle expiry and rotation.
...
- Feature flags: `session-management` enablement, staged rollout percentage, kill switch.
...
- Analytics: `session-management_started`, `session-management_succeeded`, `session-management_failed` (names illustrative—align with analytics catalog).
