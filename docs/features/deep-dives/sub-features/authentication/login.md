...
Authenticate an existing user; issue session tokens; support email/password and OAuth flows per product configuration.
...
- Feature flags: `login` enablement, staged rollout percentage, kill switch.
...
- Analytics: `login_started`, `login_succeeded`, `login_failed` (names illustrative—align with analytics catalog).
