...
Invalidate server-side session or revoke refresh tokens; clear client storage; stop push association if required.
...
- Feature flags: `logout` enablement, staged rollout percentage, kill switch.
...
- Analytics: `logout_started`, `logout_succeeded`, `logout_failed` (names illustrative—align with analytics catalog).
