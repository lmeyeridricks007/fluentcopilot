...
Device token registration; user association; logout cleanup.
...
- Feature flags: `push-registration` enablement, staged rollout percentage, kill switch.
...
- Analytics: `push-registration_started`, `push-registration_succeeded`, `push-registration_failed` (names illustrative—align with analytics catalog).
