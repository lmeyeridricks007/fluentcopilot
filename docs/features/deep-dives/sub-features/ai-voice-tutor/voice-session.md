...
Start voice session; microphone consent; create session record; entitlement checks.
...
- Feature flags: `voice-session` enablement, staged rollout percentage, kill switch.
...
- Analytics: `voice-session_started`, `voice-session_succeeded`, `voice-session_failed` (names illustrative—align with analytics catalog).
