...
Audio or text in → STT → LLM → TTS; return audio + transcript.
...
- Feature flags: `voice-turn` enablement, staged rollout percentage, kill switch.
...
- Analytics: `voice-turn_started`, `voice-turn_succeeded`, `voice-turn_failed` (names illustrative—align with analytics catalog).
