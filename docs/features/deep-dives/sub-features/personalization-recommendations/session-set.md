...
Home cards composition rules; ordering; experiments hooks.
...
- Feature flags: `session-set` enablement, staged rollout percentage, kill switch.
...
- Analytics: `session-set_started`, `session-set_succeeded`, `session-set_failed` (names illustrative—align with analytics catalog).
