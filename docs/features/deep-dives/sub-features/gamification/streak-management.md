...
Consecutive active days; freeze/skip rules if product defines them.
...
- Feature flags: `streak-management` enablement, staged rollout percentage, kill switch.
...
- Analytics: `streak-management_started`, `streak-management_succeeded`, `streak-management_failed` (names illustrative—align with analytics catalog).
