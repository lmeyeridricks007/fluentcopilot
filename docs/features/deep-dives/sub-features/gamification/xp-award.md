...
Award XP on activity completion; configurable weights; no pay-to-win.
...
- Feature flags: `xp-award` enablement, staged rollout percentage, kill switch.
...
- Analytics: `xp-award_started`, `xp-award_succeeded`, `xp-award_failed` (names illustrative—align with analytics catalog).
