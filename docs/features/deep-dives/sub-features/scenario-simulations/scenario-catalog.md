...
List scenarios by level and profile metadata; premium flags; entry to simulations.
...
- Feature flags: `scenario-catalog` enablement, staged rollout percentage, kill switch.
...
- Analytics: `scenario-catalog_started`, `scenario-catalog_succeeded`, `scenario-catalog_failed` (names illustrative—align with analytics catalog).
