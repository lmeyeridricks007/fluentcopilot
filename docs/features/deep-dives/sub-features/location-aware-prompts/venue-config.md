...
Venue types, phrase packs, geofence or static config for suggestions.
...
- Feature flags: `venue-config` enablement, staged rollout percentage, kill switch.
...
- Analytics: `venue-config_started`, `venue-config_succeeded`, `venue-config_failed` (names illustrative—align with analytics catalog).
