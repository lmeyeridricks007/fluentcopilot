...
Detect proximity or schedule; rules for when prompts may fire.
...
- Feature flags: `prompt-trigger` enablement, staged rollout percentage, kill switch.
...
- Analytics: `prompt-trigger_started`, `prompt-trigger_succeeded`, `prompt-trigger_failed` (names illustrative—align with analytics catalog).
