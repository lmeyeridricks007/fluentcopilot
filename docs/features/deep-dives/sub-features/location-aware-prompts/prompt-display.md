...
In-app card UX; dismiss; CTA to practice related content.
...
- Feature flags: `prompt-display` enablement, staged rollout percentage, kill switch.
...
- Analytics: `prompt-display_started`, `prompt-display_succeeded`, `prompt-display_failed` (names illustrative—align with analytics catalog).
