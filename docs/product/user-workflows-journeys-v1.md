...
## 4A. CEFR level, curriculum path, daily plan & revision (extension)

### 4A.1 Goal

User studies at a chosen **CEFR level** (e.g. A2, B1), follows that level’s **ordered curriculum** (units and lessons), sees a **suggested plan for today**, has all progress **saved** as today (via existing lesson checkpoints and completion), returns to **weak areas** after poor quiz performance, and runs **revision** sessions that reuse exercises from lessons they already completed.

### 4A.2 Entry points

- Onboarding or **Settings**: set or change **active study level** (aligned with profile; see feature extension docs).
- **Learn / Home**: **My path**, **Today**, **Revision** / **Practice weak areas**.

### 4A.3 Workflow (high level)

| Step | Actor | Action | System Response |
|------|--------|--------|------------------|
| 1 | User | Confirms study level (A2, B1, …) | Load curriculum manifest for `(locale, level)`; compute next incomplete lesson |
| 2 | System | Shows **path** (units, progress) and **Today** queue | Ordered cards; respect free-tier cap on *new* lesson starts (FD-02) |
| 3 | User | Starts path lesson or Today item | Same lesson run as §4 (FD-02); checkpoints persist |
| 4 | System | On quiz/exercise errors | Tag weak skills/topics when threshold met |
| 5 | User | Opens **Revision** or **Weak areas** | Build short exercise set from completed lesson pools; submit attempts |
| 6 | User | Continues daily habit | Streak/XP unchanged in principle; optional bonus for “Today done” (FD-10) |

### 4A.4 References

- `docs/feature-extensions/cefr-curriculum-path-overview.md`
- Interim content layout: `docs/curriculum/populating-level-curriculum.md`, `data/curriculum/`
...
| 2 | System | Show home: streak, XP, today's progress, recommended next (lesson/scenario/exam); optional **Today's plan** from curriculum path | FD-10, Lesson Engine; see §4A |
