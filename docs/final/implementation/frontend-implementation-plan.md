...
**Demo / try premium (pre-backend):** Until Stripe and the entitlement API exist, the app must allow users to sign up for premium and try premium features. Implement a demo flow: upsell page has "Try premium free (demo)" and plan "Select" that set premium entitlement locally (e.g. Zustand + localStorage); Settings shows plan (Free / Premium) and "End premium demo"; once premium, all premium-gated content is accessible. When backend is live, replace with real checkout and API-driven entitlement.
